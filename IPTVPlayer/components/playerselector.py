...
class PlayerSelectorWidget(Screen):
    ...
    def __init__(self, session, inList, outList, numOfLockedItems=0, groupName="", groupObj=None):
        ...
        if len(inList) >= 30 and iconSize == 100 and screenwidth > 1100:
            numOfRow, numOfCol = 4, 8
        elif len(inList) > 16 and iconSize == 100:
            numOfRow, numOfCol = 4, 5
        elif len(inList) > 12 and iconSize == 100:
            numOfRow, numOfCol = 4, 4
        elif len(inList) > 9:
            numOfRow, numOfCol = 3, 4
        elif len(inList) > 6:
            numOfRow, numOfCol = 3, 3
        elif len(inList) > 3:
        ...
        if screenwidth >= 1920:
            ...
            font = 26
            offsetCoverX, offsetCoverY = 25, 80
        markerWidth = 45 + iconSize
        markerHeight = 45 + iconSize
        disWidth = int(iconSize / 4)
        disHeight = int(iconSize / 4)
        offsetMarkerX = int(offsetCoverX - (markerWidth - iconSize) / 2)
        offsetMarkerY = int(offsetCoverY - (markerHeight - iconSize) / 2)
        tmpX = iconSize + disWidth
        tmpY = iconSize + disHeight
        ...
        skin = """
            <screen name="PlayerSelectorWidget" position="center,center" title="E2iPlayer %s" size="%d,%d" backgroundColor="#40000000">
            <widget name="statustext" position="0,0" zPosition="1" size="%d,50" font="Regular;%d" halign="center" valign="center" transparent="1"/>
            ...
            offsetCoverX + tmpX * numOfCol + offsetCoverX - disWidth,
            offsetCoverY + tmpY * numOfRow + offsetCoverX - disHeight,
            offsetCoverX + tmpX * numOfCol + offsetCoverX - disWidth,
            font,
            offsetMarkerX,
            offsetMarkerY,
        ...
        for y in range(1, numOfRow + 1):
            for x in range(1, numOfCol + 1):
                skinCoverLine = """<widget name="cover_%s%s" zPosition="4" position="%d,%d" size="%d,%d" transparent="1" alphatest="blend" />""" % (x, y, (offsetCoverX + tmpX * (x - 1)), (offsetCoverY + tmpY * (y - 1)), iconSize, iconSize)
                skin += "\n" + skinCoverLine
        ...
        self.pixmapList = [LoadPixmap(GetIconDir("PlayerSelector/" + self.currList[idx][1] + "%i.png" % self.IconsSize)) for idx in range(0, self.numOfItems)]
        ...
        self["actions"] = ActionMap(["WizardActions", "DirectionActions", "ColorActions", "IPTVPlayerListActions"], {"ok": self.ok_pressed, "back": self.back_pressed, "left": self.keyLeft, "right": self.keyRight, "up": self.keyUp, "down": self.keyDown, "blue": self.keyBlue, "menu": self.keyMenu}, -1)
        self["marker"] = Cover3()
    ...
    def calcMarkerPosY(self):
        ...
        if self.currLine == (self.numOfLines - 1):
            ...
            self.dispX = min(self.dispX, self.numItemsInLine - 1)
    ...
    def onStart(self):
        ...
        self.offsetCoverX = self["marker"].position[0] + (self.markerWidth - self.iconSize) / 2
        self.offsetCoverY = self["marker"].position[1] + (self.markerHeight - self.iconSize) / 2
    ...
    def setIdx(self, selIdx):
        selIdx = min(selIdx, self.numOfItems)
    ...
    def moveMarker(self, prev_idx=0):
        ...
        imgPosX = self.offsetCoverX + (self.iconSize + self.disWidth) * self.dispX
        imgPosY = self.offsetCoverY + (self.iconSize + self.disHeight) * self.dispY
        # calculate postion of marker for current image
        x = int(imgPosX - (self.markerWidth - self.iconSize) / 2)
        y = int(imgPosY - (self.markerHeight - self.iconSize) / 2)
    ...
    def keyMenu(self):
        ...
        if self.groupObj is not None and selItem is not None and self.groupObj.getGroupsWithoutHost(selItem[1]):
