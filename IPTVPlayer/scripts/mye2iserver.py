...
###################################################
# This module runs in its own process (started by the plugin), not inside E2iPlayer,
# so it does its own Python 2 / Python 3 handling. The same file works with both.
import sys
...
if PY2:
    ...
    import socketserver
    from urllib.parse import urlsplit, parse_qs, quote as _url_quote
    text_type = str
    string_types = str
###################################################
...
def _quote(text):
    return _url_quote(_to_bytes(text), safe='')


try:
    _same = hmac.compare_digest
except AttributeError:  # Python 2 before 2.7.7
...
def _log(text):
    """One line for the plugin's debug log. (Python 2 cannot print non-ASCII text into a pipe.)"""
...
def updateStatus(pType, pData, pCode=None):
    ...
    sys.stderr.write("\n%s\n" % json.dumps(obj))
    sys.stderr.flush()  # Python 2 keeps stderr in a buffer when it is redirected to a file
...
def _save_and_log_dump_locked(name, text):
    ...
    _log('[MyE2i-DUMP] ===== BEGIN %s (%d chars, file: %s) =====' % (safe_name, len(text), path))
    emitted = 0
    for line in text.splitlines() or ['']:
        while True:
            chunk, line = line[:DEBUG_DUMP_LOG_LINE_CHARS], line[DEBUG_DUMP_LOG_LINE_CHARS:]
            _log('[MyE2i-DUMP %s] %s' % (safe_name, chunk))
            emitted += len(chunk)
            if not line:
                break
        if emitted >= DEBUG_DUMP_LOG_MAX_CHARS:
            _log('[MyE2i-DUMP %s] ... log output truncated at %d chars, full dump is in the file above' % (safe_name, DEBUG_DUMP_LOG_MAX_CHARS))
            break
    _log('[MyE2i-DUMP] ===== END %s =====' % safe_name)
    sys.stdout.flush()
...
''' % (html_escape(token, quote=True), _json_for_html({k: T(k) for k in DEFAULT_TEXTS if k.startswith('x_')}), MIN_EXTENSION_VERSION_STR,
    ...
    DIRECTORY = os.path.join(os.path.dirname(os.path.abspath(__file__)), "htdocs")
    print(DIRECTORY)
    os.chdir(DIRECTORY)  # SimpleHTTPRequestHandler serves the current folder (Python 2 has no "directory" argument)
...
<h2>%s</h2><p>%%(code)d - %%(explain)s</p></body></html>''' % H('err_title').replace('%', '%%'))
    ...
    class RedirectHandler(SimpleHTTPRequestHandler):
        def send_error(self, code, message=None, explain=None):
            ...
            text = T(error_keys[code]) if code in error_keys else (explain or message or '')
            body = (error_page % {'code': code, 'explain': html_escape(text)}).encode('utf-8')
        ...
        def _trusted(self):
            ...
            if _same(_to_bytes(supplied), _to_bytes(token)):
        ...
        def _authorized(self):
            if self._trusted():
                return True
            _log('[MyE2i-Browser] rejected %s request from %s without a valid token' % (urlsplit(self.path).path, self.client_address[0]))
            sys.stdout.flush()
            self.send_error(403)
            return False
        ...
        def _entry(self):
            ...
            if given:
                if not pin:
                    ...
                    error = T('pin_locked')
                elif _same(_to_bytes(given), _to_bytes(pin)):
                    ...
                    pin_failures[0] += 1
                    error = T('pin_wrong')
                    _log('[MyE2i-Browser] wrong code from %s (%d/%d)' % (self.client_address[0], pin_failures[0], MAX_PIN_FAILURES))
                    sys.stdout.flush()
        ...
        def do_GET(self):
            ...
            if route == '/debug':
                ...
                self._reply('OK')
                _log('[MyE2i-Browser] ' + self._query().get('msg', [''])[0])
                sys.stdout.flush()
                return
            if route == '/version':
                ...
                if not ext_version:
                    _log('[MyE2i-Browser] Extension version check: no version reported (old extension?)')
                elif _parse_version_tuple(ext_version) < MIN_EXTENSION_VERSION:
                    _log('[MyE2i-Browser] Extension version check: v%s is OUTDATED (min v%s)' % (ext_version, MIN_EXTENSION_VERSION_STR))
                    updateStatus('status', 'MyE2i extension is outdated - please update it.')
                else:
                    _log('[MyE2i-Browser] Extension version check: v%s OK (min v%s)' % (ext_version, MIN_EXTENSION_VERSION_STR))
                sys.stdout.flush()
                return
        ...
        def do_POST(self):
            ...
            self._reply('OK')
            _save_and_log_dump(name, body.decode('utf-8', 'replace'))
    ...
    try:
        ...
        if captchaType == 'CF':
            ...
            if captchaAction:
                ...
                fragment += '&a=' + _quote(captchaAction)
            if captchaData:
                # Turnstile customer data
                fragment += '&d=' + _quote(captchaData)
            ...
            if solver_texts:
                fragment += '&l=' + _quote(json.dumps(solver_texts, ensure_ascii=False))
